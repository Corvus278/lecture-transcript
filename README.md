...
Зачем видео, а не только звук: в лекции по математике формулы существуют только на слайдах. Лектор произносит «корень из икс квадрат минус икс равно минус пять», а записать `√(x²−x) = −5` можно только по слайду.

Всё работает локально, без внешних API: в кадре вебинара видны имена участников.
...
| Качество OCR и распознавания речи, VRAM, время на GPU | ⏳ нужна машина с RTX 4060 и весами моделей |
...
Требования: Docker с NVIDIA Container Toolkit, GPU с 8 ГБ VRAM (проектировалось под RTX 4060, WSL2 + Docker на Windows).
